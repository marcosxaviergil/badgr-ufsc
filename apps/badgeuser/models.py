...
from issuer.utils import generate_sha256_hashstring  # ✅ ADICIONADO: Import da função de hashing
...
class CachedEmailAddress(EmailAddress, cachemodel.CacheModel):
    ...
    def save(self, *args, **kwargs):
        # ✅ CORREÇÃO CRÍTICA: Hash do e-mail antes de salvar
        original_email = self.email
        
        # Se o e-mail parece ser um e-mail real (contém @) e não está hasheado ainda
        if '@' in self.email and not self.email.startswith('sha256$'):
            self.email = generate_sha256_hashstring(self.email.lower())
        
        super(CachedEmailAddress, self).save(*args, **kwargs)
        
        # Usar o e-mail original (não hasheado) para as tarefas
        process_post_recipient_id_verification_change.delay(original_email, 'email', self.verified)
        
        # Adicionar variante apenas se ainda não hasheado
        if not self.emailaddressvariant_set.exists() and original_email != original_email.lower():
            self.add_variant(original_email.lower())
    ...
    def add_variant(self, email_variation):
        # ✅ CORREÇÃO: Hash da variação também
        if '@' in email_variation and not email_variation.startswith('sha256$'):
            email_variation = generate_sha256_hashstring(email_variation.lower())
...
class EmailAddressVariant(models.Model):
    email = models.CharField(max_length=254, blank=False)  # ✅ ALTERADO: CharField para suportar hash

    canonical_email = models.ForeignKey(CachedEmailAddress, blank=False)

    def save(self, *args, **kwargs):
        # ✅ CORREÇÃO: Hash do e-mail da variante
        if '@' in self.email and not self.email.startswith('sha256$'):
            self.email = generate_sha256_hashstring(self.email.lower())
    ...
    def is_valid(self, raise_exception=False):
        ...
        # ✅ CORREÇÃO: Comparar hashes em vez de e-mails em texto simples
        canonical_hash = self.canonical_email.email
        variant_hash = self.email
        
        if canonical_hash != variant_hash:
...
class BadgeUser(BaseVersionedEntity, AbstractUser, cachemodel.CacheModel):
    ...
    def set_email_items(self, value, send_confirmations=True, allow_verify=False):
        ...
        # ✅ CORREÇÃO: Hash dos e-mails antes de criar o índice
        new_email_idx = {}
        for d in value:
            email = d['email']
            if '@' in email and not email.startswith('sha256$'):
                hashed_email = generate_sha256_hashstring(email.lower())
            else:
                hashed_email = email
            new_email_idx[hashed_email] = d
        ...
        with transaction.atomic():
            ...
            for email_data in value:
                ...
                # ✅ CORREÇÃO: Hash do e-mail antes de get_or_create
                original_email = email_data['email']
                if '@' in original_email and not original_email.startswith('sha256$'):
                    hashed_email = generate_sha256_hashstring(original_email.lower())
                else:
                    hashed_email = original_email
                ...
                if not created:
                    ...
                    if emailaddress.user_id == self.id:
                        ...
                        raise ValidationError("Email '{}' may already be in use".format(original_email))
                ...
                # ✅ CORREÇÃO: Hash das variantes também
                ...
                for requested_variant in requested_variants:
                    if '@' in requested_variant and not requested_variant.startswith('sha256$'):
                        hashed_variant = generate_sha256_hashstring(requested_variant.lower())
                    else:
                        hashed_variant = requested_variant
                        
                    if hashed_variant not in existing_variant_emails:
                        EmailAddressVariant.objects.create(
                            canonical_email=emailaddress, email=hashed_variant
                        )
        ...
        # ✅ CORREÇÃO: Manter e-mail original no BadgeUser (não hasheado)
        if self.email != requested_primary['email']:
    ...
    def can_add_variant(self, email):
        # ✅ CORREÇÃO: Hash do e-mail antes de buscar
        if '@' in email and not email.startswith('sha256$'):
            hashed_email = generate_sha256_hashstring(email.lower())
        else:
            hashed_email = email
        ...
        try:
            canonical_email = CachedEmailAddress.objects.get(email=hashed_email, user=self, verified=True)
        except CachedEmailAddress.DoesNotExist:
            return False

        if hashed_email != canonical_email.email \
                and hashed_email not in [e.email for e in canonical_email.cached_variants()] \
                and EmailAddressVariant(email=hashed_email, canonical_email=canonical_email).is_valid():
            return True
        return False
    ...
    def is_email_verified(self, email):
        # ✅ CORREÇÃO: Hash do e-mail antes de verificar
        if '@' in email and not email.startswith('sha256$'):
            hashed_email = generate_sha256_hashstring(email.lower())
        else:
            hashed_email = email
            
        if hashed_email in self.all_verified_recipient_identifiers:
            return True
