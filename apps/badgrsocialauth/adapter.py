...
class BadgrSocialAccountAdapter(DefaultSocialAccountAdapter):
    ...
    def get_login_redirect_url(self, request):
        """
        ✅ ADICIONADO: Redirecionamento customizado para UFSC
        Redireciona usuários UFSC diretamente para o dashboard do frontend
        """
        # Verificar se é login social UFSC
        if hasattr(request, 'session') and 'socialaccount_state' in request.session:
            # Para UFSC, redirecionar direto para o dashboard
            return 'https://badges.setic.ufsc.br/recipient'
        
        # Para outros providers, usar comportamento padrão
