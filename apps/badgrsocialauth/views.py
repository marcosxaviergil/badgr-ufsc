...
class BadgrSocialEmailExists(RedirectView):
    def get_redirect_url(self):
        ...
        if badgr_app is not None:
            ...
            # ✅ CORREÇÃO CRÍTICA: Para UFSC, redirecionar para login success em vez de erro
            if provider == 'ufsc':
                return set_url_query_params(
                    badgr_app.ui_signup_success_redirect or 'https://badges.setic.ufsc.br/recipient',
...
class BadgrSocialAccountVerifyEmail(RedirectView):
    def get_redirect_url(self):
        ...
        # ✅ CORREÇÃO CRÍTICA: Para UFSC, ir direto para dashboard sem verificação adicional
        provider = None
        if hasattr(self.request, 'session'):
            sociallogin_data = self.request.session.get('socialaccount_sociallogin', {})
            if isinstance(sociallogin_data, dict):
                account_data = sociallogin_data.get('account', {})
                if hasattr(account_data, 'provider'):
