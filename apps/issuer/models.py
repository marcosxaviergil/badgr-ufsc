...
def get_user_or_none(recipient_id, recipient_type):
    ...
    if recipient_type == 'email':
        # TENTATIVA 1: Buscar por hash (lógica atual)
        hashed_recipient_id = generate_sha256_hashstring(recipient_id.lower())
        verified_email = CachedEmailAddress.objects.filter(verified=True, email=hashed_recipient_id).first()
        if verified_email:
            user = verified_email.user
        else:
            # TENTATIVA 2: Buscar por email original (CORREÇÃO para problema atual)
            verified_email = CachedEmailAddress.objects.filter(verified=True, email=recipient_id.lower()).first()
            if verified_email:
                user = verified_email.user
    else:
...
class BadgeClass(ResizeUploadedImage,
    ...
    def issue(self, recipient_id=None, evidence=None, narrative=None, notify=False, created_by=None, allow_uppercase=False, badgr_app=None, recipient_type=RECIPIENT_TYPE_EMAIL, **kwargs):
        # Hash do recipient_id para emails antes de salvar
        if recipient_type == RECIPIENT_TYPE_EMAIL and recipient_id:
            hashed_recipient_id = generate_sha256_hashstring(recipient_id.lower())
        else:
            hashed_recipient_id = recipient_id
        
        return BadgeInstance.objects.create(
            badgeclass=self, 
            recipient_identifier=hashed_recipient_id, 
...
class BadgeInstance(BaseAuditedModel,
    ...
    notification_email = models.EmailField(max_length=254, blank=True, null=True, 
                                         help_text="Original email address for sending notifications")
    ...
    def save(self, *args, **kwargs):
        if self.pk is None:
            ...
            if self.recipient_type == RECIPIENT_TYPE_EMAIL and '@' in self.recipient_identifier:
                self.notification_email = self.recipient_identifier
                self.recipient_identifier = generate_sha256_hashstring(self.recipient_identifier.lower())
    ...
    def notify_earner(self, badgr_app=None):
        ...
        adapter = get_adapter()
        notification_email = self.notification_email or (self.user.email if self.user else None)
        if not notification_email:
            standard_logger = logging.getLogger('issuer.models')
            standard_logger.warning(f"Cannot send notification for badge {self.entity_id}: no valid email found")
            return
    
        adapter.send_mail(template_name, notification_email, context=email_context)
    ...
    def get_recipient_display_name(self):
        """
        Retorna nome de exibição do recipient da badge
        """
        # Prioridade 1: Usuário vinculado
        if self.user:
        ...
        # Prioridade 2: Email de notificação
        if self.notification_email:
            return self.notification_email
        
        # Prioridade 3: RecipientProfile
        profile = self.cached_recipient_profile
    ...
    def recipient_display_name(self):
        return self.get_recipient_display_name()
