...
class BadgeRecipientSerializerV2(BaseSerializerV2):
    ...
    # ✅ CORREÇÃO: Usar display_name em vez de recipient_identifier para mostrar nome legível
    plaintextIdentity = serializers.SerializerMethodField(read_only=True, required=False)
    # ✅ NOVO: Campo para nome de exibição amigável
    ...
    def get_plaintextIdentity(self, instance):
        """
        ✅ CORREÇÃO: Retorna nome amigável em vez do hash
        """
        if hasattr(instance, 'get_recipient_display_name'):
            return instance.get_recipient_display_name()
        elif hasattr(instance, 'notification_email') and instance.notification_email:
            return instance.notification_email
        return "Usuário Anônimo"

    def get_displayName(self, instance):
        """
        ✅ NOVO: Campo específico para nome de exibição
        """
        if hasattr(instance, 'get_recipient_display_name'):
            return instance.get_recipient_display_name()
            ...
            'properties': OrderedDict([
                ...
                ('recipient', {
                    ...
                    'properties': OrderedDict([
                        ...
                        ('plaintextIdentity', {
                            'type': 'string',
                            'description': "The plaintext identity or display name",
                            'required': False,
                        }),
                        ('displayName', {
                            'type': 'string',
                            'description': "Human-readable display name for the recipient",
                            'required': False,
                        }),
                    ]),
