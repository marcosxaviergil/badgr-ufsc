...
class Command(BaseCommand):
    help = 'Configura dados iniciais do sistema Badgr UFSC (BadgrApp, OAuth, External Tools)'
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                # 5. Configurar External Tools (funcionalidade original)
    ...
    def _setup_external_tools(self, force_recreate):
        """Configurar External Tools (funcionalidade original)"""
        self.stdout.write('\n🔧 Configurando External Tools...')
        
        base_url = self._get_base_url()
        
        # Criar external tool para OAuth UFSC usando variável de ambiente
        external_tool, created = ExternalTool.objects.get_or_create(
            name="UFSC OAuth Login",
            defaults={
                'label': "Login UFSC",
                'launch_url': f"{base_url}/auth/login/ufsc-oauth2/",
                'launch_type': "link",
                'is_enabled': True
            }
        )
        ...
        if not created and (force_recreate or f"{base_url}/auth/login/ufsc-oauth2/" != external_tool.launch_url):
            external_tool.launch_url = f"{base_url}/auth/login/ufsc-oauth2/"
            external_tool.save()
        
        action = 'criado' if created else ('atualizado' if force_recreate else 'já existe')
        self.stdout.write(f'  ✓ External Tool {action}: {external_tool.name}')
        self.stdout.write(f'    URL: {external_tool.launch_url}')
    ...
    def _show_summary(self):
        ...
        # External Tools
        self.stdout.write('\n🔧 External Tools:')
        for tool in ExternalTool.objects.all():
            status = '✅' if tool.is_enabled else '❌'
            self.stdout.write(f'  - {status} {tool.name}: {tool.launch_url}')
