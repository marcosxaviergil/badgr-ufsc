...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            import allauth
            self.stdout.write(f"   django-allauth: {allauth.__version__}")
        except:
            self.stdout.write("   django-allauth: versão não detectada")
        
        try:
            import corsheaders
            self.stdout.write(f"   django-cors-headers: {corsheaders.__version__}")
        except:
            self.stdout.write("   django-cors-headers: versão não detectada")
        
        # 3. Verificar backends
        self.stdout.write("\n3. Testando backends:")
        for backend_path in settings.AUTHENTICATION_BACKENDS:
            try:
                ...
                self.stdout.write(f"   ✓ {backend_path}")
            except Exception as e:
                self.stdout.write(f"   ✗ {backend_path} - ERRO: {e}")
        
        # 4. Verificar Site
        self.stdout.write("\n4. Configuração de Sites:")
        try:
    ...
    def test_authentication(self, email, password):
        self.stdout.write(f"\n7. Testando autenticação para {email}:")
        ...
        user_model = backend.authenticate(username=email, password=password)
        if user_model:
        ...
        try:
            ...
            user_allauth = auth_backend.authenticate(username=email, password=password)
            if user_allauth:
