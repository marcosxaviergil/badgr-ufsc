...
class EmailBlacklist(cachemodel.CacheModel):
    ...
    def verify_email_signature(email_encoded, expiration, signature):
        ...
        hashed = hmac.new(secret_key.encode('utf-8'), email_encoded + expiration.encode('utf-8'), sha1)
    ...
    def generate_new_token_for_user(self, user, scope='r:profile', application=None, expires=None, refresh_token=False):
        with transaction.atomic():
            ...
            if expires is None:
                ...
                expires = timezone.now() + timedelta(seconds=access_token_expires_seconds)
