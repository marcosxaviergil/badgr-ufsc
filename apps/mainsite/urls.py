...
# Redirecionamento de login para OAuth2
@csrf_exempt
def login_redirect(request):
    """Redireciona login do frontend para nosso endpoint OAuth2"""
    if request.method == 'POST':
        # Redirecionar POST de login para nosso endpoint
        return token_login(request)
    else:
        # GET retorna info para frontend saber onde fazer login
        return JsonResponse({
            'loginUrl': '/api/v2/auth/token-login/',
            'method': 'POST',
            'message': 'Use token-based authentication'
        })

urlpatterns = [
    # ✅ CORREÇÃO CRÍTICA: Interceptar URL incorreta do frontend
    url(r'^account/sociallogin$', RedirectView.as_view(
        url='/accounts/sociallogin?provider=ufsc', 
        permanent=False
    ), name='frontend_sociallogin_fix'),

    # Redirecionamentos de login do frontend
    ...
    url(r'^accounts/login/$', login_redirect, name='account_login'),  # Interceptado
