...
# ✅ CALLBACK REDIRECT ESPECÍFICO PARA UFSC
class UfscCallbackRedirect(RedirectView):
    def get_redirect_url(self, *args, **kwargs):
        # Preservar todos os parâmetros da query string
        query_string = self.request.META.get('QUERY_STRING', '')
        api_callback_url = f"https://api-badges.setic.ufsc.br/accounts/ufsc/login/callback/"
        if query_string:
            api_callback_url += f"?{query_string}"
        return api_callback_url

urlpatterns = [
    # ✅ INTERCEPTAR CALLBACK DO FRONTEND E REDIRECIONAR PARA API (ANTES DE TUDO!)
    url(r'^accounts/ufsc/login/callback/$', UfscCallbackRedirect.as_view(), name='ufsc_callback_redirect'),
