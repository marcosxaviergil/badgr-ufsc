...
class RecipientProfile(BaseVersionedEntity, CreatedUpdatedAt, CreatedUpdatedBy, IsActive):
    ...
    recipient_identifier = models.CharField(max_length=1024)
    ...
    def save(self, *args, **kwargs):
        # Se recipient_identifier parece ser um email e não está hasheado, fazer o hash
        if '@' in self.recipient_identifier and len(self.recipient_identifier) < 100:  # emails são menores que 100 chars, hashes são 64
            self.recipient_identifier = generate_sha256_hashstring(self.recipient_identifier.lower())
        super(RecipientProfile, self).save(*args, **kwargs)

    @property
    def jsonld_id(self):
        # Como recipient_identifier agora é um hash, não podemos usar mailto: diretamente
        # Usar um identificador genérico baseado no hash
        return 'urn:hash:{}'.format(self.recipient_identifier)
